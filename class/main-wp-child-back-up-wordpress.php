...
class Main_WP_Child_Back_Up_WordPress {
	...
	/**
	 * @return Main_WP_Child_Back_Up_WordPress
	 */
	static function instance() {
		if ( null === Main_WP_Child_Back_Up_WordPress::$instance ) {
	...
	public function action() {
		...
		// @TODO: Nonce verification check
		if ( isset( $_POST['mwp_action']) ) { // @codingStandardsIgnoreLine
			switch ( $_POST['mwp_action'] ) { // @codingStandardsIgnoreLine
				case 'set_showhide':
	...
	function check_schedule() {
		// @TODO: Nonce verification check
		$schedule_id = ( isset( $_POST['schedule_id'] ) && ! empty( $_POST['schedule_id'] ) ) ? $_POST['schedule_id'] : ''; // @codingStandardsIgnoreLine
	...
	function reload_backups() {
		...
		// @TODO: SANITIZE
		$schedule_ids = isset( $_POST['schedule_ids'] ) ? $_POST['schedule_ids'] : array(); // @codingStandardsIgnoreLine
		// @TODO: Check if class exists
		HM\BackUpWordPress\Schedules::get_instance()->refresh_schedules();
	...
	function hmbkp_request_delete_backup() {
		...
		if ( ! isset( $_POST['hmbkp_backuparchive'] ) || empty( $_POST['hmbkp_backuparchive'] ) ) { // @codingStandardsIgnoreLine
		...
		$deleted = $schedule->delete_backup( sanitize_text_field( base64_decode( $_POST['hmbkp_backuparchive'] ) ) ); // @codingStandardsIgnoreLine
	...
	function get_backupslist_html( $schedule ) {
		// @TODO: Check if function exists: hmbkp_backups_number
		ob_start();
		?>
		<table class="widefat">
			<thead>
			...
			if ( $schedule->get_backups() ) {
				...
				?>
				<tr>
					<td class="hmbkp-no-backups" colspan="4"><?php esc_html_e( 'This is where your backups will appear once you have some.', 'mainwp-backupwordpress-extension' ); ?></td>
				</tr>
				<?php
			}
			?>
	...
	function hmbkp_get_site_size_text( HM\BackUpWordPress\Scheduled_Backup $schedule ) {
		// @TODO: Refactor to work if HM\BackUpWordPress\Scheduled_Backup doesn't exist
	...
	function hmbkp_get_backup_row( $file, HM\BackUpWordPress\Scheduled_Backup $schedule ) {
		// @TODO: Refactor to work if HM\BackUpWordPress\Scheduled_Backup doesn't exist
		// @TODO: Check if hmbkp_human_get_type, hmbkp_is_path_accessible, hmbkp_path, mainwp_backupwp_download_backup exists
		...
		<tr class="hmbkp_manage_backups_row">
			<th scope="row">
				<?php echo esc_html( date_i18n( get_option( 'date_format' ) . ' - ' . get_option( 'time_format' ), $file_mod_time + $offset ) ); ?>
			</th>
			<td class="code">
				<?php echo esc_html( size_format( $file_size ) ); ?>
			</td>
			<td>
				<?php echo esc_html( hmbkp_human_get_type( $file, $schedule ) ); ?>
			</td>
			<td>
				<?php if (  hmbkp_is_path_accessible( hmbkp_path() )  ) : ?>
					<a href="#" onclick="event.preventDefault(); mainwp_backupwp_download_backup('<?php echo esc_attr( $encoded_file ); ?>', <?php echo esc_attr( $schedule->get_id() ); ?>, this);" class="download-action"><?php esc_html_e( 'Download', 'backupwordpress' ); ?></a> |
				<?php endif; ?>
				<a href="#" onclick="event.preventDefault(); mainwp_backupwp_delete_backup('<?php echo esc_attr( $encoded_file ); ?>', <?php echo esc_attr( $schedule->get_id() ); ?>, this);"  class="delete-action"><?php esc_html_e( 'Delete', 'backupwordpress' ); ?></a>
			</td>
		</tr>
		<?php
	}
	...
	function get_excluded( $browse_dir = null ) {
		...
		<?php if ( $schedule->get_excludes() ) : ?>
			<h3><?php esc_html_e( 'Currently Excluded', 'backupwordpress' ); ?></h3>
			<?php // @TODO: Refactor this as a sprintf, so it's translatable ?>
			...
			<table class="widefat">
				<tbody>
					<?php foreach ( array_diff( $schedule->get_excludes(), $schedule->backup->default_excludes() ) as $key => $exclude ) : ?>
						<?php $exclude_path = new SplFileInfo( trailingslashit( $schedule->backup->get_root() ) . ltrim( str_ireplace( $schedule->backup->get_root(), '', $exclude ), '/' ) ); ?>
						<tr>
							<th scope="row">
							...
							<td>
								...
								<?php else : ?>
									<a href="#" onclick="event.preventDefault(); mainwp_backupwp_remove_exclude_rule('<?php echo esc_attr( $exclude ); ?>', this);"  class="delete-action"><?php esc_html_e( 'Stop excluding', 'backupwordpress' ); ?></a>
								<?php endif; ?>
							</td>
						</tr>
					<?php endforeach; ?>
		...
		if ( $files ) {
				...
				<tr>
								...
								<?php else : ?>
									...
									if ( false !== $size ) :
										...
										<code>
											...
											<?php // @TODO: Refactor into a multiline sprintf ?>
											<a class="dashicons dashicons-update" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'hmbkp_recalculate_directory_filesize', urlencode( $schedule->backup->get_root() ) ) ), 'hmbkp-recalculate_directory_filesize' ); ?>"><span><?php esc_html_e( 'Refresh', 'backupwordpress' ); ?></span></a>
										</code>
										<?php
									endif;
								endif;
								?>
							<?php // @TODO: Missing a </td> ?? ?>
							<td>
					...
					<tbody>
						<?php foreach ( $files as $size => $file ) {
							...
							/* @codingStandardsIgnoreStart */
							$path_name = @realpath( $file->getPathname() );
							/* @codingStandardsIgnoreEnd */
							if ( ! $path_name || ! $file->isReadable() ) {
								$is_unreadable = true;
							}
							?>
							<tr>
								...
								<td>
									<?php if ( $is_unreadable ) : ?>
										<code class="strikethrough" title="<?php echo esc_attr( $file->getRealPath() ); ?>">
											<?php echo esc_html( $file->getBasename() ); ?>
										</code>
									<?php elseif ( $file->isFile() ) : ?>
										<code title="<?php echo esc_attr( $file->getRealPath() ); ?>">
											<?php echo esc_html( $file->getBasename() ); ?>
										</code>
									<?php elseif ( $file->isDir() ) : ?>
										<code title="<?php echo esc_attr( $file->getRealPath() ); ?>">
											<a href="#" onclick="event.preventDefault(); mainwp_backupwp_directory_browse('<?php echo urlencode( $file->getPathname() ); ?>', this)" ><?php echo esc_html( $file->getBasename() ); ?></a>
										</code>
									<?php endif; ?>
								</td>
									...
									<?php else : ?>
										...
										if ( false !== $size ) :
											...
											<code>
												<?php echo esc_html( $size ); ?>
												<?php if ( $file->isDir() ) : ?>
													<a title="<?php esc_html_e( 'Recalculate the size of this directory', 'backupwordpress' ); ?>" class="dashicons dashicons-update" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'hmbkp_recalculate_directory_filesize', urlencode( $file->getPathname() ) ) ), 'hmbkp-recalculate_directory_filesize' ); ?>">
								...
								<td>
									<?php if ( $file->isLink() ) : ?>
										<span title="<?php echo esc_attr( $file->GetRealPath() ); ?>"><?php esc_html_e( 'Symlink', 'backupwordpress' ); ?></span>
									<?php elseif ( $file->isDir() ) : ?>
										<?php esc_html_e( 'Folder', 'backupwordpress' ); ?>
									<?php else : ?>
										<?php esc_html_e( 'File', 'backupwordpress' ); ?>
									<?php endif; ?>
								</td>
								<td class="column-format">
									<?php if ( $is_unreadable ) : ?>
										<strong title="<?php esc_html_e( 'Unreadable files won\'t be backed up.', 'backupwordpress' ); ?>">
									...
									<?php else : ?>
										...
										?>
										<a href="#" onclick="event.preventDefault(); mainwp_backupwp_exclude_add_rule('<?php echo urlencode( $exclude_path ); ?>', this)" class="button-secondary">
			...
			<?php } ?>
			<p class="submit">
				<a href="#" onclick="event.preventDefault(); mainwp_backupwp_edit_exclude_done()" class="button-primary">
	...
	function directory_browse() {
		// @TODO: Nonce verification check
		// @TODO: SANITIZE
		$browse_dir = $_POST['browse_dir']; // @codingStandardsIgnoreLine
		$out = array();
	...
	function hmbkp_add_exclude_rule() {
		// @TODO: Nonce verification check
		// @TODO: SANITIZE
		if ( ! isset( $_POST['exclude_pathname'] ) || empty( $_POST['exclude_pathname']) ) { // @codingStandardsIgnoreLine
		...
		$exclude_rule = urldecode( $_POST['exclude_pathname'] ); // @codingStandardsIgnoreLine
		...
		$out['e'] = $return['e'];
		$out['current_browse_dir'] = $_POST['browse_dir']; // @codingStandardsIgnoreLine
		return $out;
	}
	...
	function hmbkp_remove_exclude_rule() {
		// @TODO: Nonce verification check
		// @TODO: SANITIZE
		if ( ! isset( $_POST['remove_rule'] ) || empty( $_POST['remove_rule']) ) { // @codingStandardsIgnoreLine
			return array( 'error' => __( 'Error: Empty exclude directory path.' ) );
		}
		...
		$schedule->set_excludes( array_diff( $excludes, (array) stripslashes( sanitize_text_field( $_POST['remove_rule'] ) ) ) ); // @codingStandardsIgnoreLine
	...
	function update_schedule() {
		// @TODO: Nonce verification check
		// @TODO: SANITIZE
		$sch_id  = isset( $_POST['schedule_id']) ? $_POST['schedule_id'] : 0; // @codingStandardsIgnoreLine
		$sch_id  = sanitize_text_field( urldecode( $sch_id ) );
		$options = isset ($_POST['options'] ) ? unserialize( base64_decode( $_POST['options'] ) ) : false; // @codingStandardsIgnoreLine
