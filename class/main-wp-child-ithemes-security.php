...
class Main_WP_Child_iThemes_Security {
	...
	/**
	 * @return Main_WP_Child_iThemes_Security
	 */
	static function instance() {
		if ( null === Main_WP_Child_iThemes_Security::$instance ) {
	...
	public function __construct() {
		...
		add_action( 'mainwp-site-sync-others-data', array( $this, 'sync_others_data' ) );
	}
	 ...
	 * @param $data
	 */
	function sync_others_data( $data ) {
	...
	function save_settings() {
		...
		if ( isset( $settings['itsec_tweaks'] ) ) {
			...
			// @TODO: Refactor next three statements, $enable_set_safe_jquery doesn't need to exist
			$enable_set_safe_jquery = false;
			...
			if ( update_site_option( 'itsec_tweaks', $settings['itsec_tweaks'] ) ) {
				// @TODO: Input may not have been defined
		...
		// @TODO: Check if ITSEC_Lib exists
		$site_status = array(
	...
	public function whitelist() {
		global $itsec_globals;
		// @TODO: SANITIZE and Nonce
		$ip = $_POST['ip']; // @codingStandardsIgnoreLine
		$add_temp   = false;
		$temp_ip    = get_site_option( 'itsec_temp_whitelist_ip' );
		if ( false !== $temp_ip ) {
			if ( ( $temp_ip['exp'] < $itsec_globals['current_time']) || $ip !== $temp_ip['exp'] ) {
	...
	function admin_user() {
		// @TODO: Nonce
		//Process admin user
		$username    = isset( $_POST['admin_username'] ) ? trim( sanitize_text_field( $_POST['admin_username'] ) ) : null; // @codingStandardsIgnoreLine
		// @TODO: Check this line carefully, looks like a logic mistake
		$change_id_1 = isset( $_POST['admin_userid'] ) && intval( $_POST['admin_userid'] == 1 ) ? true : false; // @codingStandardsIgnoreLine
		...
		if ( $change_id_1 ) {
			// @TODO: SANITIZE and Nonce
			$user = get_user_by( 'login', $_POST['user'] ); // @codingStandardsIgnoreLine
			if ( 1 === $user->ID ) {
	...
	private function change_admin_user( $username = null, $id = false ) {
		...
		if ( $itsec_files->get_file_lock( 'admin_user' ) ) { //make sure it isn't already running
			...
			if ( null !== $username && validate_username( $new_user ) && username_exists( $new_user ) === null ) { //there is a valid username to change
				if ( true === $id ) { //we're changing the id too so we'll set the username
					$user_login = $new_user;
				} else { // we're only changing the username
					//query main user table
					// @TODO: SANITIZE with $wpdb->prepare or $wpdb->update
	...
	public function process_database_prefix() {
		...
		foreach ( $tables as $table ) {
			...
			// @TODO: SANITIZE with $wpdb->prepare
			// @TODO: $new_prefix might not have been defined
			if ( $wpdb->query( 'RENAME TABLE `' . $wpdb->base_prefix . $table . '` TO `' . $new_prefix . $table . '`;' ) === false ) {
				$type    = 'error';
				$message = sprintf(
					'%s %s%s. %s',
					__( 'Error: Could not rename table', 'it-l10n-better-wp-security' ),
					$wpdb->base_prefix,
					$table,
					__( 'You may have to rename the table manually.', 'it-l10n-better-wp-security' )
				);
	...
	public function release_lockout() {
		...
		// @TODO: SANITIZE and Nonce
		$lockout_ids = $_POST['lockout_ids']; // @codingStandardsIgnoreLine
