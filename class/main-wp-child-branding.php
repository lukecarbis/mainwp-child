...
class Main_WP_Child_Branding
{
	...
	if ( isset($_POST['submit']) ) {
		...
		if ( $this->send_support_mail() ) {
			...
			} else { 					$send_email_message = 'Your Message was successfully submitted.'; }
		} else {
	...
	public function add_support_button_in_top_admin_bar( $wp_admin_bar ) {
		...
		if ( isset($_GET['from_page']) ) {
			$href = admin_url( 'admin.php?page=ContactSupport&from_page=' . urlencode( esc_url( $_GET['from_page'] ) ) ); } else {
			$protocol = isset($_SERVER['HTTPS']) && strcasecmp( $_SERVER['HTTPS'], 'off' ) ? 'https://' : 'http://';
			$fullurl = $protocol .$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
