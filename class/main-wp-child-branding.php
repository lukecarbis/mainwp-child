...
class Main_WP_Child_Branding {
	...
	static function instance() {
		if ( null === Main_WP_Child_Branding::$instance ) {
	...
	public function plugin_row_meta( $plugin_meta, $plugin_file, $child_plugin_slug ) {
		...
		$total_meta = count( $plugin_meta );
		for ( $i = 0; $i < $total_meta; $i++ ) {
	...
	public function update_branding() {
		...
		$information = array();
		$settings = unserialize( base64_decode( $_POST['settings'] ) ); // @codingStandardsIgnoreLine
		...
		if ( isset( $settings['child_login_image_url'] ) ) {
			if ( empty( $settings['child_login_image_url'] ) ) {
				...
				try {
					$upload = $this->upload_image( $settings['child_login_image_url'] ); //Upload image to WP
					if ( null !== $upload ) {
		...
		if ( isset( $settings['child_favico_image_url'] ) ) {
			if ( empty( $settings['child_favico_image_url'] ) ) {
				...
				try {
					$upload = $this->upload_image( $settings['child_favico_image_url'] ); //Upload image to WP
					if ( null !== $upload ) {
	 ...
	 * @throws Exception
	 */
	static function upload_image( $img_url ) {
		// @TODO: If we have to include this file, we're probably doing it wrong
	...
	public function branding_init() {
		...
		$cancelled_branding = ( 'yes' === get_option( 'mainwp_child_branding_disconnected' ) ) && ! get_option( 'mainwp_branding_preserve_branding' );
		...
		if ( 'Y' !== get_option( 'mainwp_branding_disable_wp_branding' ) ) {
			...
			if ( isset( $extra_setting['hide_nag'] ) && ! empty( $extra_setting['hide_nag'] ) ) {
				// @TODO: Refactor, don't use an eval'ed function
	...
	function custom_login_logo() {
		...
		if ( isset( $extra_setting['login_image']['url'] ) && ! empty( $extra_setting['login_image']['url'] ) ) {
			...
			<style type="text/css">
				h1 a {
					background-image: url( '<?php echo esc_url( $login_logo ); ?>' ) !important;
	...
	function custom_gettext( $translations, $text, $domain = 'default' ) {
		// @TODO: $text and $domain aren't used, remove
		unset( $text );
		unset( $domain );
		$extra_setting = $this->settings['extra_settings'];
	...
	public function send_support_mail() {
		...
		$content = wp_kses_post( nl2br( stripslashes( $_POST['mainwp_branding_contact_message_content'] ) ) ); // @codingStandardsIgnoreLine
		if ( ! empty( $_POST['mainwp_branding_contact_message_content'] ) && ! empty( $email ) ) { // @codingStandardsIgnoreLine
			global $current_user;
			$mail = '<p>Support Email from: <a href="' . site_url() . '">' . site_url() . '</a></p>';
			// @TODO: Refactor this line as sprintf
			$mail .= '<p>Sent from WordPress page: ' . ( ! empty( $_POST['mainwp_branding_send_from_page'] ) ? '<a href="' . esc_url( $_POST['mainwp_branding_send_from_page'] ) . '">' . esc_url( $_POST['mainwp_branding_send_from_page'] ) . '</a></p>' : ''); // @codingStandardsIgnoreLine
			...
			// @TODO: Logic mistake needs fixing
			if ( $mail_result ) { } // @codingStandardsIgnoreLine
				return true;
		}
		return false;
	}
	...
	function contact_support() {
		...
		if ( isset( $_POST['submit'] ) ) {
			...
			$from_page = $_POST['mainwp_branding_send_from_page']; // @TODO: Sanitize straight away
			...
			?>
			<div class="mainwp_info-box-yellow"><?php echo esc_html( $send_email_message ). '&nbsp;&nbsp' . wp_kses_post( $back_link ); ?></div>
			<?php
		} else {
			if ( isset( $_GET['from_page'] ) ) {
				$from_page = urldecode( $_GET['from_page'] ); // @TODO: SANITIZE
			} else {
			...
			<form action="" method="post">
				<div style="width: 99%;">        
					<h2><?php echo wp_kses_post( $this->settings['contact_support_label'] ); ?></h2>
			...
			if ( $this->check_update_child_plugin() ) {
				...
				<script>
					jQuery( document ).ready(function() {
						...
						var plugin_count = jQuery( 'span.update-plugins > span.plugin-count' );
						if ( menu_count ) {
	...
	public function branding_map_meta_cap( $caps, $cap, $user_id, $args ) {
		// @TODO: $user_id and $args are unsued, remove
		unset( $user_id );
		unset( $args );
